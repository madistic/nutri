...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, doc, updateDoc, deleteDoc, where, getDocs } from 'firebase/firestore';
import { Calendar, Utensils, Activity, Droplet, History, Home, Loader2, PlusCircle, Trash2, Edit, Camera, Flame, Candy, ChevronDown, ChevronUp, Target } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, BarChart, Bar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar } from 'recharts';

import React, { useState, useRef, useEffect } from 'react';
import { initializeApp } from 'firebase/app';
import { getAuth, signInAnonymously, onAuthStateChanged, signOut } from 'firebase/auth'; // Removed explicit login/register methods
import { getFirestore, doc, setDoc, getDoc, collection, onSnapshot, addDoc, serverTimestamp, query, orderBy } from 'firebase/firestore';
import { Sparkles, MessageSquare, Mic, Volume2, Zap, Crown, Globe, Users, Home, ChevronRight, X, Play, Pause, Send, ArrowLeft, Bot, Menu, ChevronLeft, ChefHat, BarChart2, Scan, Database, TrendingUp, Lightbulb, Droplet, Dumbbell, Repeat, Camera, NotebookPen, Calculator, Beef, ClipboardList, Tally3, Image as ImageIcon, MapPin, Cloudy, Utensils, DollarSign, Search, Wind, Flame, Wheat, BrainCircuit, BookOpen, HeartPulse, ShoppingCart, HelpCircle, Sun, CloudRain, Snowflake, UserPlus } from 'lucide-react'; // Removed LogIn, UserPlus, LogOut icons
...
// Removed LoginPage and RegisterPage components

const HomePage = ({ onNavigate }) => ( // Removed isAuthenticated prop
    <div className="p-8 text-center min-h-[500px] flex flex-col justify-center items-center">
        ...
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
            {/* Always show Start Chatting button */}
            <button
...
function NutriBotApp() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (user) {
                setUserId(user.uid);
                await fetchUserProfile(db, user.uid);
                setCurrentPage('home'); 
            } else {
                // If no user (e.g., first load or previous anonymous session expired)
                // Sign in anonymously to ensure a persistent user ID for Firestore
                try {
                    const anonymousUserCredential = await signInAnonymously(auth);
                    setUserId(anonymousUserCredential.user.uid);
                    await fetchUserProfile(db, anonymousUserCredential.user.uid);
                    setCurrentPage('home');
                } catch (anonError) {
                    console.error("Error signing in anonymously:", anonError);
                    // Fallback if anonymous sign-in also fails (should be rare)
                    setUserId(crypto.randomUUID()); // Generate a random ID as a last resort
                }
            }
            setAuthReady(true);
        });
    ...
    const saveUserProfile = async (profile) => {
        ...
        const docRef = doc(dbRef.current, `/artifacts/${appId}/users/${userId}/profile/info`);
        try {
    ...
    // Removed handleLogin, handleRegister, handleLogout functions
    ...
    const handleSendMessage = async (message = input) => {
        if ((message.trim() === '' && !imageUpload) || !authReady) return;
        ...
        const newUserMessage = { sender: 'user', text: message.trim(), image: imagePreview };
        ...
        if (!authReady) {
            return (
                <div className="flex flex-col items-center justify-center h-[500px] text-gray-400">
                    <Sparkles className="w-16 h-16 animate-spin text-purple-400" />
                    <p className="mt-4 text-lg">Loading NutriBot AI...</p>
                    <p className="text-sm">Initializing Firebase and getting ready.</p>
                </div>
            );
        }

        // Removed conditional rendering for login/register pages
        switch (currentPage) {
            case 'home':
                return <HomePage onNavigate={setCurrentPage} />; // Removed isAuthenticated prop
            case 'chat':
                return <ChatPage
                    messages={messages}
                    input={input}
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-indigo-900 flex items-center justify-center p-4 font-inter text-white">
            <div className="relative w-full max-w-4xl bg-gray-800 bg-opacity-70 backdrop-filter backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden border border-purple-700/50 flex flex-col">
                ...
                {showProfileModal && <ProfileModal onSubmit={saveUserProfile} onClose={() => setShowProfileModal(false)} />}
                ...
                <div className="p-6 w-full flex flex-col">
                    <div className="border-b border-purple-700/50 pb-6 mb-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            {/* Always show profile setup button */}
                            <button
...
const App = () => {
    ...
    const [currentPage, setCurrentPage] = useState('dashboard'); // 'dashboard', 'glucose', 'food', 'exercise', 'history', 'imageAnalysis', 'imageAnalysisHistory', 'goals'
    ...
    useEffect(() => {
        ...
        if (typeof __firebase_config === 'string' && __firebase_config.trim() !== '') {
            try {
                ...
                console.error("Failed to parse __firebase_config. It might be malformed JSON:", e);
                firebaseConfig = {};
            }
        } else {
            console.warn("__firebase_config is not a valid string or is empty. Using default empty config.");
            firebaseConfig = {};
        }
    ...
    return (
        <AppContext.Provider value={{ db, auth, userId, showCustomModal }}>
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
        const userGlucosePath = `artifacts/${appId}/users/${userId}/glucoseReadings`;
        const userFoodPath = `artifacts/${appId}/users/${userId}/foodEntries`;
        const userExercisePath = `artifacts/${appId}/users/${userId}/exerciseEntries`;
        const userImageAnalysisPath = `artifacts/${appId}/users/${userId}/imageAnalysisResults`;
        const userGoalsPath = `artifacts/${appId}/users/${userId}/goals`;
...
const GoalsPage = ({ showCustomModal }) => {
    ...
    useEffect(() => {
        ...
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
        const paths = {
            goals: `artifacts/${appId}/users/${userId}/goals`,
            glucose: `artifacts/${appId}/users/${userId}/glucoseReadings`,
            food: `artifacts/${appId}/users/${userId}/foodEntries`,
            exercise: `artifacts/${appId}/users/${userId}/exerciseEntries`,
        };
...
const HistoryPage = ({ showCustomModal }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
        const glucosePath = `artifacts/${appId}/users/${userId}/glucoseReadings`;
        const foodPath = `artifacts/${appId}/users/${userId}/foodEntries`;
        const exercisePath = `artifacts/${appId}/users/${userId}/exerciseEntries`;
    ...
    const handleDelete = async (entry) => {
        showCustomModal(`Are you sure you want to delete this ${entry.type} entry?`, async () => {
            try {
                ...
                switch (entry.type) {
                    case 'glucose':
                        collectionPath = `artifacts/${appId}/users/${userId}/glucoseReadings`;
                        break;
                    case 'food':
                        collectionPath = `artifacts/${appId}/users/${userId}/foodEntries`;
                        break;
                    case 'exercise':
                        collectionPath = `artifacts/${appId}/users/${userId}/exerciseEntries`;
                        break;
                    default:
                        return;
                }
    ...
    const analyzeImage = async () => {
        ...
        reader.onloadend = async () => {
            ...
            try {
                ...
                if (db && userId) {
                    const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
                    await addDoc(collection(db, `artifacts/${appId}/users/${userId}/imageAnalysisResults`), {
                        timestamp: new Date().toISOString(),
...
const ImageAnalysisHistory = ({ showCustomModal }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
        const q = query(collection(db, `artifacts/${appId}/users/${userId}/imageAnalysisResults`), orderBy('timestamp', 'desc'));
        const unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    const handleDelete = (id) => {
        showCustomModal("Are you sure you want to delete this analysis?", async () => {
            try {
                const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
                await deleteDoc(doc(db, `artifacts/${appId}/users/${userId}/imageAnalysisResults`, id));
